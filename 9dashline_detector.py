...
model = YOLO(r"E:\HD_VNese_map\main_src\yolov8n\runs\detect\train12\weights\best.pt")
...
for img_idx, image_filename in enumerate(image_list):
	...
	results = model.predict(source=image_dir, save_txt=False, conf=0.2 , iou=0.2, show=True, device = "cpu")	
	...
	if saving_command == 's':
		results = model.predict(source=image_dir, save_txt=True, conf=0.2, iou=0.2, show=False, device = "cpu")	
