...
def extract_frames(video_path, frames_dir, overwrite=False, start=-1, end=-1, every=1):
    ...
    while frame < end:  # lets loop through the frames until the end
        ...
        if while_safety > 1000:  # break the while if our safety maxs out at 500
            break
        ...
        if frame % every == 0:  # if this is a frame we want to write out based on the 'every' argument
            ...
            if not os.path.exists(save_path) or overwrite:  # if it doesn't exist or we want to overwrite anyways
                ...
                image_list.append(frames_dir)
        frame += 1  # increment our frame count

    capture.release()  # after the while has finished close the capture

    return image_list  # and return the count of the images we saved
# Define a function to collect results in a separate process

def collect_results(results_queue):
    results = []
    with concurrent.futures.ThreadPoolExecutor() as thread_executor:
        for future in concurrent.futures.as_completed(results_queue):
...
def video_to_frames(video_path, frames_dir, overwrite=False, every=1, chunk_size=1000):
    ...
    with ProcessPoolExecutor(max_workers=multiprocessing.cpu_count()) as executor:
        ...
        # Start the result collection process
        result_collector = multiprocessing.Process(target=collect_results, args=(results_queue,))
        result_collector.start()

        # Add the futures to the results queue
        for future in futures:
            results_queue.put(future)

        # Wait for the result collector to finish
        result_collector.join()

    # Retrieve the collected results from the result collector process
    collected_results = results_queue.get()
...
if __name__ == '__main__':
    ...
    video_to_frames(video_path='../yolov8n/22.mp4', frames_dir='extracted_frames', overwrite=True, every=5, chunk_size=3000)
